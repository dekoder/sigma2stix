...
def clone_github_repository(repo_url, destination_path):
    try:
...
def generate_all_references(data:dict) -> List[dict]:
    if not data.get("references", None):
        return []
    return [
        {"source_name": "reference", "url": reference}
        for reference in data.get("references", None)
    ]
